...
3. Make Onedrive available in your Virtual Lab machine. (You will only have to do this once. In future when you access the Virtual Lab, OneDrive will automatically be available.)
        * Click and approve My Files in the bar above the main desktop display:
...
5. Use `File | New File` to create a new file. This will be a _Python source code file_ containing our first Python program. Python files should have a name that ends in `.py`. Before typing the program in, save the file using `File | Save As...`. Name the file `greetings.py`, save it in your `class01` folder on OneDrive. OneDrive should be available under `This PC`:   
